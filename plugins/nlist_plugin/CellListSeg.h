...
/*! \file CellListSeg.h
    \brief Declares the CellListSeg class
*/
...
#ifndef __CellListSeg_H__
#define __CellListSeg_H__
...
/*! \b Overview:
   ...
   neighbor list. The data layout design decisions for CellListSeg were made to optimize the
   performance of the neighbor list build as determined by microbenchmarking. However, CellListSeg is
   ...
   appropriate Index2D from getCellListSegIndexer() or getCellAdjIndexer(). \code Index2D
   cell_list_indexer = cl->getCellListSegIndexer(); Index2D cell_adj_indexer = cl->getCellAdjIndexer();
    \endcode
   ...
   conditions if they choose.)
    ...
    Condition flags are to be set during the computeCellListSeg() call and will be checked by compute()
...
*/
class PYBIND11_EXPORT CellListSeg : public Compute
    {
    public:
    //! Construct a cell list
    CellListSeg(std::shared_ptr<SystemDefinition> sysdef, std::pair<unsigned int, unsigned int> segment);
    ...
    const std::pair<unsigned int, unsigned int> getSegment() const
        {
        return m_segment;
        }
    ...
    /*! \param func Function to call when the cell width changes
        ...
        \a func will be called every time the CellListSeg is notified of a change in the cell width
    ...
    */
    ...
    GlobalArray<uint3> m_conditions; //!< Condition flags set during the computeCellListSeg() call

    bool m_sort_cell_list;   //!< If true, sort cell list
    bool m_compute_adj_list; //!< If true, compute the cell adjacency lists

    std::pair<unsigned int, unsigned int> m_segment;
    ...
    virtual void computeCellListSeg();
...
namespace detail
    {
//! Export the CellListSeg class to python
#ifndef __HIPCC__
