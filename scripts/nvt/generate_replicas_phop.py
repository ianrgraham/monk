...
parser = argparse.ArgumentParser(
    description=
    "From a traj file and a specified frame run a number of realizations of that system and compute the phop for each particle"
)
...
parser.add_argument("idx",
                    nargs="+",
                    type=int,
                    help=f"Index to use for traj. Supply as many as wanted")
parser.add_argument("--replicas",
                    type=int,
                    default=1000,
                    help=f"Number of replicas to simulate")
parser.add_argument(
    "--pair",
    nargs="+",
...
parser.add_argument("--temp",
                    type=float,
                    default=1.5,
                    help="Starting and ending temperatures")
parser.add_argument("--dump-rate",
                    type=float,
                    default=0.1,
                    help="Rate at which to store snapshots")
parser.add_argument("--sim-time",
                    type=int,
                    default=10,
                    help="Time to simulate for each replica")
parser.add_argument("--seed",
                    type=int,
                    help="Random seed to initialize the RNG")
parser.add_argument("--run-fire",
                    action="store_true",
                    help="Randomize particle velocities at the start")
...
for jdx in traj_idx:
    ...
    for idx in range(replicas):
        ...
        all_phops.append([
            np.uint16(jdx),
            np.arange(len(phop[0]), dtype=np.uint16),
            np.uint16(idx), phop[0].astype(np.float32)
        ])  # trivial unwrapping

        snap_buffer.clear()

df = pd.DataFrame(all_phops, columns=["frame", "id", "replica",
                                      "phop"]).explode(["id", "phop"])
