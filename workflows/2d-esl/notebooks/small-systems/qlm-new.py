...
#     print((N*dim, N*dim))
#     hessian = ssp.dok_array((N*dim, N*dim), dtype=np.float32)
...
@njit
def _make_coo_data(edges, grad2_us, edge_vecs, dim, N, row, col, data):
    ...
    for edge_idx in np.arange(len(edges)):
        ...
        # don't forget the prefactor of 1/2 from overcounting
        # k_vec = 0.5 * grad2_us[edge_idx] * edge_vecs[edge_idx]
        # k_outer = np.outer(k_vec, k_vec).ravel()

        # TODO: I think the above code is a bug
        k_vec  = edge_vecs[edge_idx]
        k_outer = 2.0 * grad2_us[edge_idx] * np.outer(k_vec, k_vec).ravel()
...
def _compute_csr_hessian(edges, grad2_us, edge_vecs, dim, N):
    ...
    _make_coo_data(edges, grad2_us, edge_vecs, dim, N, row, col, data)
...
class QLM():
    ...
    def compute(self,
        ...
        if dense:
            ...
            hessian_csr = _compute_csr_hessian(edges, grad2_us, unit_vecs, dim, N)
    ...
    def compute_hessian(self,
        ...
        if dense:
            ...
            hessian = _compute_csr_hessian(edges, grad2_us, unit_vecs, dim, N)
